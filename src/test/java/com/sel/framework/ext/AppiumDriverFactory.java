	...
	public static AppiumDriver getAppiumDriver(MobileExecutionPlatform executionPlatform, String deviceName,
		...
		try {
			...
			case WEB_ANDROID:
				...
				desiredCapabilities.setCapability("chromedriverExecutable","/usr/lib/driver/chromedriver");
				desiredCapabilities.setCapability("platformVersion", version);
...
//					driver = new RemoteWebDriver(new URL(appiumURL), desiredCapabilities);
					AndroidDriver androiddriver = new AndroidDriver(new URL(appiumURL), desiredCapabilities);
			...
			case WEB_ANDROID:
				...
				desiredCapabilities.setCapability("platformVersion", version);
				desiredCapabilities.setCapability("browserName", "Chrome");
//				System.out.println("inside web_android :"+appiumURL);
				try {
					driver = new RemoteWebDriver(new URL(appiumURL), desiredCapabilities);
				} catch (MalformedURLException e) {
					e.printStackTrace();
					throw new FrameworkException(
							"WEB_ANDROID: The android driver invokation has problem, please check the capabilities or Start Appium");
				}
				break;
